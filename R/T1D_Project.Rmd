...
source("/Users/ryan_summers/GitHub/teddy-dnam-analysis/scripts/lmeR_traj_model.R")
source("/Users/ryan_summers/GitHub/teddy-dnam-analysis/scripts/lmeR_traj_comparisons.R")
```
...
### Pheno Complete
```{r}
pheno_complete <- read_csv("/Users/ryan_summers/GitHub/teddy-dnam-analysis/data/processed/pheno_complete.csv")
```
...
```
=======
### Linear vs Non-linear Runs

```{r}
options(lme4.show_singular_warnings = FALSE)
t0 <- Sys.time()
...
# grid based on age in years
age_grid <- seq(
  min(pheno_complete$age_yrs, na.rm = TRUE),
  max(pheno_complete$age_yrs, na.rm = TRUE),
  length.out = 12)
...
benchmark_res <- suppressWarnings(
  future_lapply(benchmark_probes, function(p) {
  compare_age_models_one_cpg(
    probe = p,
    matrix = matrix.clean,
    pheno = pheno_complete,
...
>>>>>>> 678fc14 (created new scripts to handle non-linear lmeR model fits to run on Alpine. Updated markdown file to run sample runs to determine if non-linear fits will improve over linear.)
...
lmeR_spline_results <- read_csv("/Users/ryan_summers/GitHub/teddy-dnam-analysis/results/lmeR_spline_results.csv") %>% select(-1)
```

```{r}
# spline with DF=3 - random intercept only
lmeR_spline_int_res <- read_csv("/Users/ryan_summers/GitHub/teddy-dnam-analysis/results/lmeR_spline_int_res.csv") %>% select(-1)
```
...
nrow(lmeR_MAD_results[lmeR_MAD_results$is_singular==TRUE,]) / 
  nrow(lmeR_MAD_results)

nrow(lmeR_spline_results[lmeR_spline_results$is_singular==TRUE,]) / 
  nrow(lmeR_spline_results)

nrow(lmeR_spline_int_res[lmeR_spline_int_res$is_singular==TRUE,]) / 
  nrow(lmeR_spline_int_res)
```
