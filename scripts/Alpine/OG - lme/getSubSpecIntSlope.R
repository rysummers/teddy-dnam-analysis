...
mod.function = function(
  ...
  message <- NA
  tryCatch({
    model = lme(form, random = form_rand, data=forAnalysis)
...
# error catching script
safe_mod_call_rich <- function(probe, matrix, pheno, sample_var, id_var, age_var, covs) {
  warn_msgs <- character(0)
  
  out <- tryCatch(
    withCallingHandlers(
      mod.function(probe, matrix, pheno, sample_var, id_var, age_var, covs),
      warning = function(w) {
    ...
    error = function(e) {
      data.frame(
        ...
        warning_messages = if (length(warn_msgs)) paste(unique(warn_msgs), collapse = " | ") else NA_character_,
        stringsAsFactors = FALSE
      )
    }
  )
  ...
  if (is.null(out)) {
    out <- data.frame(
      ...
      flag = 1,
      stage = "returned_null",
      message = "mod.function returned NULL (often due to warning handler aborting inside tryCatch)",
      warning_messages = if (length(warn_msgs)) paste(unique(warn_msgs), collapse = " | ") else NA_character_,
      stringsAsFactors = FALSE
    )
  } else {
    if (!("stage" %in% names(out))) out$stage <- NA_character_
    if (!("warning_messages" %in% names(out))) out$warning_messages <- NA_character_
    out$warning_messages <- if (length(warn_msgs)) paste(unique(warn_msgs), collapse = " | ") else NA_character_
  }
